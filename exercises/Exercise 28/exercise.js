...
*/
function displayCsvAsAxisColourBar(csvUrl, columnName, callback) {
    ...
    // Use the data once loaded
        ...
        // Create gradient
        let grad = d3.scaleLinear().domain([d3.min(data), d3.max(data)]).range(["deepskyblue", "orangered"]);

        // Create coloured bar
        g.append("rect")
            .attr("width", (d) => scale(d) )
            .attr('fill', (d) => grad(d) ) // Switch colour based on unscaled value
...
displayCsvAsAxisColourBar("https://raw.githubusercontent.com/ChrisJMurdoch/DataAnalytics/master/data/part_nine_data_a.csv", "value", function() {

    // Call function the second time with no callback
    d3.select("body").append("h3").text("Part 6 scene data").style("margin-top", "0");
    displayCsvAsAxisColourBar("https://raw.githubusercontent.com/ChrisJMurdoch/DataAnalytics/master/data/part_nine_data_b.csv", "value");
});
